...
public class ConvertXsams2Hitran {
	...
	/**
	 * Converts all XSAMSData d into HITRAN format and printing them into a file
	 * in path.
	 * 
	 * @param d
	 * @param path
	...
	public String extractData(XSAMSData d, String path) {
		...
		/* Output file */
		///WriteOutputFile convertedFile = new WriteOutputFile();
		//convertedFile.openFile(path + ".par");
		...
		for (RadiativeTransitionType radiative : d.getProcesses().getRadiative().getRadiativeTransitions()) {
			...
			/*
			 * Case where Sources are global for all radiative transitions Case
			 * with Wiadis database
			 */
			...
			for (DataType energy : Type) {
				...
				if (!radiative.getBroadenings().isEmpty()) {
					...
					for (BroadeningType broadening : radiative.getBroadenings()) {
						...
						try {
							...
							System.out.println("Something went wrong with lineshapes, lineshapeparameters or EnvRef.");
							return "";
						}
						...
						if (funcType.getFitParameters().isEmpty()) {
							...
							/*
							 * FIXME: should we alos test here for delta and n ?
							 * Need EXAMPLES, because only ICB data come here
							 * for now
							 */
				...
				// display(data);
				//convertedFile.writeFile(data);
				result.append(WriteOutputFile.getHitranAsString(data)+"\n");
			}
		}
		/*try {
			convertedFile.closeFile();
			System.out.println("Complete.");
		} catch (IOException e) {
			System.out.println("Something went wrong while closing file!");
		} finally {
			System.out.println("");
		}*/
		}catch(Exception e){
